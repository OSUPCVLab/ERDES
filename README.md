# A Benchmark Video Dataset for Retinal Detachment and Macular Status Classification in Ocular Ultrasound
<div align="center">
...
```bash
# this will run hyperparameter search defined in `configs/hparams_search/erdes_optuna.yaml`
# over chosen experiment config
python train.py -m hparams_search=erdes_optuna experiment=example
```

> **Note**: Using [Optuna Sweeper](https://hydra.cc/docs/next/plugins/optuna_sweeper) doesn't require you to add any boilerplate to your code, everything is defined in a [single config file](configs/hparams_search/erdes_optuna.yaml).
...
```bash
python train.py tags=["erdes","experiment_X"]
```

> **Note**: You might need to escape the bracket characters in your shell with `python train.py tags=\["erdes","experiment_X"\]`.
...
You can run DDP on erdes example with 4 GPUs like this:
...
DVC stores information about the added file (or a directory) in a special .dvc file named data/erdes.dvc, a small text file with a human-readable format. This file can be easily versioned like source code with Git, as a placeholder for the original data:

```bash
git add data/erdes.dvc data/.gitignore
...
```python
from project_name.models.erdes_module import erdesLitModule
from project_name.data.erdes_datamodule import erdesDataModule
```
